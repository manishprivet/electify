...
const App: React.FC<{ data: dataInterface }> = ({ data }) => {
  const router = useRouter();
  const getWinners = (candidates: Array<{ name: string; votes: number }>) => {
    const maximumVotes = Math.max(...candidates.map((o) => o.votes));
      ...
      {data.voters_remaining ? (
        data.auth_type === 'gsuite' ? (
          <h2 style={{ textAlign: 'center' }}>
            <span style={{ color: 'var(--button-color)' }}>{data.total_votes}</span> people of your
            domain have voted till now and
            <br />
            {winners.length > 1 ? (
              <span>
