...
export default ({ data }: { data: dataInterface }) => {
  const router = useRouter();
  const getWinners = (candidates: Array<{ name: string; votes: number }>) => {
  ...
  return (
    <div className='container'>
      ...
      {data.candidates.map((candidate, index) => (
        <div key={candidate.name + index} className='candidate c-flex'>
          <div className='candidate-label'>
            <p>{candidate.name}</p>
            <p>
              {candidate.votes} Vote{candidate.votes === 1 ? "" : "s"} out of{" "}
              {data.total_votes}
            </p>
          </div>
      ...
      {data.voters_remaining ? (
        data.auth_type === "gsuite" ? (
          <h2 style={{ textAlign: "center" }}>
            <span style={{ color: "var(--button-color)" }}>
              {data.total_votes}
            </span>{" "}
            people of your domain have voted till now and
            <br />
            {winners.length > 1 ? (
              <span>
                {winners.map((winner, i) => (
                  <span
                    key={i}
                    style={{ color: "var(--button-color)", fontSize: "1.2em" }}
                  >
                    {winner.name}
                    <span
                      style={{ color: "var(--text-color)", fontSize: "1em" }}
                    >
                      {i === winners.length - 2
                        ? " and "
                        : i === winners.length - 1
          ...
          <h2>
            ...
            out of{" "}
            <span style={{ color: "var(--button-color" }}>
              {data.voters_remaining + data.total_votes}
            </span>{" "}
        ...
        <h2 style={{ textAlign: "center", marginTop: "40px" }}>
          Election ended and there is a tie between
          <br />
          {winners.map((winner, i) => (
            <span
              key={i}
        ...
        <h2 style={{ textAlign: "center", marginTop: "40px" }}>
          Election ended and the winner is
          <br />
          <span style={{ color: "var(--button-color", fontSize: "1.2em" }}>
            {winners[0].name}
          </span>
        </h2>
      )}
