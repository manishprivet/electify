...
export default ({ createElection }) => {
  ...
  const createElectionData = () => {
    ...
    if (!pattern.test(electionId))
      return setErrors(`Election id can only contain alphabets`);
    if (parseInt(noOfVoters) > 10000)
      return setErrors(`Number of voters can't be more than`);
    const c = [...candidates];
    c.forEach((candidate, index) =>
      candidate.name === "" ? c.splice(index, 1) : null
    );
    const data = {
      ...
      <div className='candidates'>
        ...
        {candidates.map((candidate, index) => (
          <div key={`candidate-${index}`} className='candidate'>
            <input
              onKeyPress={(e) =>
                e.charCode === 13 ? newCandidateBox.current.focus() : null
              }
